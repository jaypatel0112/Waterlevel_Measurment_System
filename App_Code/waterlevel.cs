...
/// <summary>
/// Summary description for waterlevel
/// </summary>
public class waterlevel
{
    ...
    public static Boolean checkEmail(string email)
    {
        ...
        SqlCommand cmd2 = new SqlCommand("select count(*) from vendorregi where Email like '" + email + "'", cn);
    ...
    public static Boolean createUser(string username, string type, string name, string img, string pass = "")
    {
        ...
        if (s == 0)
        {
            ...
            mail(username, "Thank,you For your Registration On SD ", "Your username is " + username + "<br>your Password is " + pass + "<br>You can Login by clicking <a href='" + basepath + "log-in.aspx'>HERE</a>");
            state = true;
        }
        else
        {
    ...
    public static void mail(string mailto, string subject, string msg)
    {
        ...
        SMTPServer.Credentials = new System.Net.NetworkCredential("info@brainbean.in", "Dhruvik!23");

        SMTPServer.Send(mailObj);
    }
    public static string sqlDisplaytable(string query, string link = "", string btntext = "View More", string link2 = "", string btn2txt = "", string link3 = "", string btn3txt = "", string link4 = "", string btn4txt = "")
    {
        SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString); if (cn.State == ConnectionState.Open)
            cn.Close();
        cn.Open();
        ...
        if (1 == 1)
        {
            ...
            //tabel start
            data = "<table class='table table-bordered table-striped table-vcenter'>";
            ...
            while (dr.Read())
            {
                ...
                {
                    string s = "";
                    if (dr.GetName(i).ToString() == "status" || dr.GetName(i).ToString() == "STATUS" || dr.GetName(i).ToString() == "Status")
                    {
                        ...
                        if (dr[dr.GetName(i).ToString()].ToString().Trim() == "Pending")
                            ...
                            s = "warning";
                        else if (dr[dr.GetName(i).ToString()].ToString().Trim() == "Canceled" || dr[dr.GetName(i).ToString()].ToString().Trim() == "Declined")
                            s = "danger";
                ...
                if (link2 != "")
                    data = data + "<td><a class='btn btn-danger' href='" + link2 + "" + dr[dr.GetName(0).ToString()].ToString() + "'> " + btn2txt + "</a></td>";
                if (link3 != "")
                    data = data + "<td><a class='btn btn-warning' href='" + link3 + "" + dr[dr.GetName(0).ToString()].ToString() + "'> " + btn3txt + "</a></td> ";
                if (link4 != "")
                    data = data + "<td><a class='btn btn-success' href='" + link4 + "" + dr[dr.GetName(0).ToString()].ToString() + "'> " + btn4txt + "</a></td>";
                else
                    data = data + "</tr>";
            }
            data = data + "</tbody>";
        ...
        if (c == 0)
        {
            data = "<h4 style='text-align: center;'>No Data Found</h4><img src='" + basepath + "nodata.gif' style='text-align: center;display: block; margin-left: auto; margin-right: auto; width: 50%;'  class='img-responsive'>";
        }
    ...
    {
        HttpWebRequest myReq = (HttpWebRequest)WebRequest.Create("http://api.msg91.com/api/sendhttp.php?sender=SOLDOC&route=4&mobiles=" + mobno + "&authkey=208338A910w4rhF5qk5ac9e620&country=91&message=" + msg);
    ...
    public static Boolean forgetpass(string email)
    {
        SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString); if (cn.State == ConnectionState.Open)
            cn.Close();
        cn.Open();
        string pass = waterlevel.passgen(8);
        ...
        try
        {
            ...
            mail(email, "Expel Prosys Password has been Updated", "Your new Password is " + pass);
        }
        catch
        {
    ...
    public static Boolean autheticateUser(string id, string pass)
    {
        ...
        cn.Open();
        string r = "";
        SqlCommand cmd = new SqlCommand("select * login where email like '" + id + "' AND Password like '" + pass + "';", cn);
    ...
    public static void sendmsg(string senderid, string reciverid, string msg)
    {
        ...
        waterlevel.insert("messages", DateTime.Now.ToString(), msg, senderid, reciverid, "1", "0");
    ...
    public static string GenrateCalenderEvents(string userid)
    {
        ...
        while (dr.Read())
        {
            ...
            try
            {
                status = waterlevel.getSingleData("select status from schedule where id=" + dr["taskname"].ToString());
                if (status == "Pending")
                    color = "#ffca28";
            ...
            if (i == 0)
            {
                ...
                data = "{title:'" + dr["text"].ToString() + "',start:'" + d.ToString("yyyy-MM-ddTHH:mm:sszzz") + "',end:'" + e.ToString("yyyy-MM-ddTHH:mm:sszzz") + "',color:'" + color + "',backgroundColor:'" + color + "', url:'" + basepath + "/task/schedule/details.aspx?id=" + dr["taskname"].ToString() + dr["taskid"].ToString() + "'}";
            }
            else
            {
                data = data + ",{title:'" + dr["text"].ToString() + "',start:'" + d.ToString("yyyy-MM-ddTHH:mm:sszzz") + "',end:'" + e.ToString("yyyy-MM-ddTHH:mm:sszzz") + "',color:'" + color + "',backgroundColor:'" + color + "',url:'" + basepath + "/task/schedule/details.aspx?id=" + dr["taskname"].ToString() + dr["taskid"].ToString() + "'}";
            }
            i++;
        }
        cn.Close();
    ...
    public static string genrateemployeelist(string jobs, string sid = "")
    {
        ...
        if (sid != "")
            seduleempname = waterlevel.getSingleData("select assignedemps from schedule where id=" + sid);

        else
            seduleempname = waterlevel.getSingleData("select assignedemps from schedule where jobcode like '" + jobs + "'");
        ...
        {
            if (sid != "")
                n = Int32.Parse(waterlevel.getSingleData("select (LEN(assignedemps)- LEN(REPLACE(assignedemps, ',', '')))+1 from schedule where id=" + sid));

            else
                n = Int32.Parse(waterlevel.getSingleData("select (LEN(assignedemps)- LEN(REPLACE(assignedemps, ',', '')))+1 from schedule where jobcode like '" + jobs + "'"));
        }
        catch
        {
            n = 0;
        ...
        while (i < n)
        {
            ...
            try
            {
                ...
                emp[i] = waterlevel.getSingleData("select fname+' '+lname from employee where id=" + emp[i]);
            }
            catch
            {
                emp[i] = seduleempname;
    ...
    public static string genratedata(string seduleempname, string tablename, string colunmname)
    {
        ...
        while (i < n - 1)
        {
            ...
            {
                emp[i] = seduleempname.Substring(0, seduleempname.IndexOf(","));
                emp[i] = waterlevel.getSingleData("select " + colunmname + " from " + tablename + " where id=" + emp[i]);
                seduleempname = seduleempname.Substring(seduleempname.IndexOf(",") + 1);
    ...
    public static string genrateemployeethumb(string jobs)
    {
        ...
        string seduleempname = waterlevel.getSingleData("select assignedemps from schedule where id=" + jobs);
        string name = seduleempname;
        string[] emp = new string[100];
        int i = 0;
        string cname = "";
        ...
        string team = waterlevel.getSingleData("select teamleader from schedule where id=" + jobs);
        data = data + "<li><a href='" + basepath + "task/employee/details.aspx?id=" + waterlevel.getSingleData("select teamleader from schedule where id=" + jobs) + "'> <img class='img-avatar img-avatar-thumb' src='" + waterlevel.getSingleData("select photo from employee where id=" + team) + "' alt=''>" + waterlevel.getSingleData("select fname+' '+lname from employee where id=" + team) + " &nbsp &nbsp <span class='badge badge-pill badge-danger'>Team Leader</span></a> </li>";
        int n;
        try
        {
            n = Int32.Parse(waterlevel.getSingleData("select (LEN(assignedemps)- LEN(REPLACE(assignedemps, ',', '')))+1 from schedule where id=" + jobs));
        }
        catch
        {
            n = 0;
        ...
        while (i < n)
        {
            ...
            try
            {
                emp[i] = seduleempname.Substring(0, seduleempname.IndexOf(","));
                seduleempname = seduleempname.Substring(seduleempname.IndexOf(",") + 1);
                img = waterlevel.getSingleData("select photo from employee where id=" + emp[i]);
                id = emp[i];
                emp[i] = waterlevel.getSingleData("select fname+' '+lname from employee where id=" + emp[i]);
